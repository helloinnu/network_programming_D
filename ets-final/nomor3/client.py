...
def getresultpemain(index,result):
    ...
    result = getdatapemain(random.randint(1, 20),True)
    if (result):
...
if __name__ == '__main__':
    thread_count = int(sys.argv[1]) if len(sys.argv) >= 2 else 20
    ...
    print(f'Total Thread   : {thread_count}')
    print(f'Total Request  : {request_count}')
    print(f'Total Response : {response_count}')
    print(f'Latency        : {(time_end - time_start) * 1000:.3f} ms')